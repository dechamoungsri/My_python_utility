...
class Utility(object):
    ...
    @staticmethod
    def inteporate_lf0(lf0):

        y = np.copy(lf0)
        x = np.arange(len(lf0))

        unvoice_index = np.where(lf0<0)

        y = np.delete(y, unvoice_index)
        x = np.delete(x, unvoice_index)

        lf0[unvoice_index] = np.interp(unvoice_index, x, y)[0]

        # print 'interp :', np.interp(unvoice_index, x, y)[0]

        return lf0

    @staticmethod
    def get_date_and_time_now():
        return '{}:{}'.format(datetime.datetime.now().date(), datetime.datetime.now().time())
...
#     import shutil
    ...
    @staticmethod
    def dct_noise_reduction(y, coeff_number = 5):
